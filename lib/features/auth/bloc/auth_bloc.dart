...
enum AuthStatus { unknown, unauthenticated, profileIncomplete, authenticated }
...
final class AuthState extends Equatable {
  ...
  const AuthState.profileIncomplete(User user)
    : this(status: AuthStatus.profileIncomplete, user: user);
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onStarted(AuthStarted event, Emitter<AuthState> emit) async {
    ...
    emit(_stateForUser(currentUser));
  }
  ...
  AuthState _stateForUser(User user) {
    if (user.needsProfileCompletion) {
      return AuthState.profileIncomplete(user);
    }
